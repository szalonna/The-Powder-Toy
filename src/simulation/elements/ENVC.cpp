...
int Element_ENVC_update(UPDATE_FUNC_ARGS);
int Element_ENVC_graphics(GRAPHICS_FUNC_ARGS);

void Element::Element_ENVC()
{
	Identifier = "DEFAULT_PT_ENVC";
	Name = "ENVC";
	...
	HeatConduct = 251;
	Description = "ENVCanium. Stores energy and releases it in violent explosions.";
	...
	Update = &Element_ENVC_update;
	Graphics = &Element_ENVC_graphics;
}
...
int Element_ENVC_update(UPDATE_FUNC_ARGS)
{
	...
	// 		{
	// 			r = pmap[y+ry][x+rx];
	// 			if (TYP(r) && TYP(r)!=PT_ENVC  && TYP(r)!=PT_BVBR && sim->elements[TYP(r)].HeatConduct && (TYP(r)!=PT_HSWC||parts[ID(r)].life==10))
	// 			{
		...
		for (ry=-1; ry<2; ry++)
			if (BOUNDS_CHECK && (rx || ry))
			{
				...
				if (parts[i].life)
				{
					...
					if ((TYP(r)==PT_ENVC  || TYP(r)==PT_BVBR))
					{
						if (!parts[ID(r)].life)
							parts[ID(r)].tmp += 45;
